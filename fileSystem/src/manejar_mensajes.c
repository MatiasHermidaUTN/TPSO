...
int manejar_mensaje() {
	...
	switch (args->cod_op) {
		...
		case TRUNCAR_ARCHIVO:
			nombre_archivo = strdup(args->parametros[0]);
			nuevo_tamanio_archivo = atoi(args->parametros[1]);
			char* pid_truncar = strdup(args->parametros[2]);
		...
		case ESCRIBIR_ARCHIVO:
			...
			string_array_push(&parametros_a_enviar_escribir, nombre_archivo); //nombre del archivo
