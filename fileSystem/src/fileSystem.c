...
int main(int argc, char** argv) {
	...
	t_log* logger = iniciar_logger("FileSystem.log", "FS");
	...
	//SE HACE SERVIDOR Y ESPERDA LA CONEXION DEL KERNEL
	...
	tamanoBitmap = (int) ceil( c/8.0 ); 	// tener en cuenta si no se necesita en otro lado
	...
	//directorio_FCB = config_create("directorio_FCB");
	//BLOQUES
	bloques = fopen(lectura_de_config., "r+")
	if (bloques){
		log_info(logger, "Bloques existe");
	...
	while (1) {
		...
		msync(bitmap_pointer, tamanoBitmap, MS_SYNC);
		munmap(bitmap_pointer, tamanoBitmap);
		fclose(bloques);
		return 0;
	}
	config_destroy(superbloque);
...
void leer_indirecto(char** buffer, uint32_t puntero_indirecto, int cuanto_leer) {
	...
	t_list_iterator *lista_bloques = list_iterator_create();
	...
	uint32_t dir_bloque = list_iterator_index(0);
	while (true) {
		...
		if(list_iterator_has_next(lista_bloques))
			dir_bloque = list_iterator_next(lista_bloques);
		else
			break;
	}
	free(buffer1);
...
void escribir_archivo(char* buffer, char* nombre_archivo, int apartir_de_donde_escribir, int cuanto_escribir) {
	...
	if(checkear_espacio(cuanto_escribir))//chequea que pueda escribi todo!!!!
	{
		if (tamanio_archivo == 0){		//solo para archivos NUEVOS
			cuanto_escribir_string = intToCharAsterisco((unsigned int) cuanto_escribir);
			config_set_value(archivo_FCB, "TAMANIO_ARCHIVO", cuanto_escribir_string);

			uint32_t puntero_directo = dame_un_bloque_libre();
			puntero_directo_string = intToCharAsterisco((unsigned int) puntero_directo);
			...
			if(cuanto_escribir > 0){
				...
				puntero_indirecto_string = intToCharAsterisco((unsigned int) puntero_indirecto);
			...
			if(espacio_libre_en(puntero_directo) > 0){
				escribir_bloque(&buffer, puntero_directo, &cuanto_escribir, &cantidad_escrita, espacio_libre_en(puntero_directo));
			}
		}
	}
...
char* intToCharAsterisco(unsigned int numero){
	...
	int digitos = cantidad_de_digitos(numero), aux;
...
void escribir_bloque(char* buffer, uint32_t puntero, int* cuanto_escribir, int* cantidad_escrita, int espacio_libre_en_bloque) {
	...
	fseek(bloques, puntero * super_bloque_info.block_size + apartir_de_donde_escribir, SEEK_SET);
	char* copia_buffer = string_substring(buffer, *cantidad_escrita, *cuanto_escribir);
